defmodule VestaboardAgent.E2E.BoardRateTest do
  ...
  describe "board rate limiting" do
        ...
        Enum.map(Enum.with_index(frames), fn {grid, i} ->
          ...
          case Client.write_characters(grid) do
            {:ok, _} ->
              ...
              IO.puts(
                "  frame #{i}: write=#{dispatch_ms}ms  confirm=#{confirm_ms}ms  total=#{total_ms}ms  #{status}"
              )

              %{
                frame: i,
                write_ms: dispatch_ms,
    ...
    test "measures raw write throughput with 1s between frames" do
      ...
      IO.puts(
        "  [rate] write time  min=#{Enum.min(all_ms)}ms  max=#{Enum.max(all_ms)}ms  p50=#{percentile(all_ms, 50)}ms"
      )
  ...
  defp print_stats(timings) do
    ...
    IO.puts(
      "  total    min=#{Enum.min(totals)}ms  max=#{Enum.max(totals)}ms  p50=#{percentile(totals, 50)}ms  p95=#{percentile(totals, 95)}ms"
    )

    if confirms != [] do
      IO.puts(
        "  confirm  min=#{Enum.min(confirms)}ms  max=#{Enum.max(confirms)}ms  p50=#{percentile(confirms, 50)}ms  p95=#{percentile(confirms, 95)}ms"
      )
    end
